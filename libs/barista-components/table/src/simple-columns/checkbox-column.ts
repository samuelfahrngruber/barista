...
export class DtCheckboxColumn<T> extends DtSimpleColumnBase<T> {
  _isAllSelected(): boolean {
    const numSelected = this._selection._selectionModel.selected.length;
    const numRows = (this.table.dataSource as DtTableDataSource<T>).data.length;
    return numSelected == numRows;
  }

  _isAllSelected$ = this._selection._selectionModel.changed.pipe(
    map(() => this._isAllSelected()),
  );

  _isAnySelected(): boolean {
    return this._selection._selectionModel.hasValue() && !this._isAllSelected();
  }

  _isAnySelected$ = this._selection._selectionModel.changed.pipe(
    map(() => this._isAnySelected()),
  );
  /**
  ...
  @Input() rowDisabledAccessorFn: Predicate<T> = () => false;

  constructor(private _selection: DtSelection<T>, table: DtTable<T>) {
    super(table);
  }

  ngDoCheck(): void {
    console.log('oh do check');
  }

  _isSelected(row: T): boolean {
    return this._selection._selectionModel.isSelected(row);
  }

  _toggleRow(changeEvent: DtCheckboxChange<T>, row: T): void {
    if (changeEvent.checked) {
      this._selection._selectionModel.select(row);
    } else {
      this._selection._selectionModel.deselect(row);
    }
  }

  _toggleAllSelection(changeEvent: DtCheckboxChange<T>): void {
    const data = (this.table.dataSource as DtTableDataSource<T>).data;
    if (changeEvent.checked) {
      this._selection._selectionModel.select(...data);
    } else {
      this._selection._selectionModel.clear();
    }
  }
